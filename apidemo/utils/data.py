...
def get_dev_test_encodings(
    ...
    **kwargs):
    ...
    weakly_supervised_data_file = os.path.join(root, "data", f"weakly_supervised_data_cluster.jsonl")
    weakly_supervised_dev_file = os.path.join(root, "data", f"weakly_supervised_dev_cluster.jsonl")
...
def get_dataset(
    ...
    **kwargs) -> Tuple[Union[IDataset, None], Union[IDataset, None], IDataset]:
    ...
    example_file = os.path.join(root, "examples.json") #f"{dataset}.example.char.jsonl")
    label_info_file = os.path.join(root, "label_info.json")
    if setting == "span":
        train_file = os.path.join(root, f"{dataset}.train.span.jsonl")
        dev_file = os.path.join(root, f"{dataset}.dev.span.jsonl")
        ...
        dev_file = os.path.join(root, f"{dataset}.dev.char.jsonl")
        if not os.path.exists(dev_file):
            dev_file = os.path.join(root, "data", f"weakly_supervised_dev_cluster.jsonl")
        test_file = os.path.join(root, f"{dataset}.test.char.jsonl")
        if not os.path.exists(test_file):
            test_file = os.path.join(root, "data", f"weakly_supervised_dev_cluster.jsonl")
    if setting == "span":
        weakly_supervised_data_file = os.path.join(root, "data", f"weakly_supervised_data_cluster.span.jsonl")
    else:
        weakly_supervised_data_file = os.path.join(root, "data", f"weakly_supervised_data_cluster.jsonl")
    weakly_supervised_dev_file = os.path.join(root, "data", f"weakly_supervised_dev_cluster.jsonl")
    nearest_example_file = os.path.join(root, "data", f"train_data_nearest_examples.json")
